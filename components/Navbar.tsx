    ...
    <nav className="sticky top-0 z-50 glass-nav bg-white/70 dark:bg-background-dark/70 border-b border-pink-100 dark:border-pink-900/30 transition-colors">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium">
          ...
          <Link to="/personalizar/new" className="bg-primary text-white px-5 py-2 rounded-full font-bold hover:bg-pink-600 transition-colors shadow-lg shadow-pink-200 dark:shadow-none">
            Personalizar Ahora
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/carrito" className="p-2 relative rounded-full hover:bg-pink-100 dark:hover:bg-pink-900/30 transition-colors">
            ...
            {cartCount > 0 && (
              <span className="absolute top-1 right-1 bg-primary text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center animate-bounce-short">
                {cartCount}
              </span>
            )}
          </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white dark:bg-background-dark border-t border-pink-100 dark:border-pink-900/30 py-4 px-4 flex flex-col gap-4 animate-in slide-in-from-top duration-300">
          ...
          <Link to="/personalizar/new" onClick={() => setIsMenuOpen(false)} className="py-2 text-primary font-bold flex items-center gap-2">
            <span className="material-icons-round">edit</span> Personalizar Ahora
          </Link>
